    ...
    <div className="w-full h-full pb-2">
      ...
      <div className="carousel carousel-center space-x-3 h-[220px] mt-1">
        {ARListData3D.map((mission, index) => (
          ...
          <div className="carousel-item relative border-[1px] rounded-lg drop-shadow-sm ml-4">
            <div className="absolute top-1 left-2 bg-white py-1 px-2 rounded-lg border-2 font-bold text-sm">
              <p>{mission.point} คะแนน</p>
            </div>
            <img src={mission.ar_url} className="absolute z-20 w-32 right-4 top-4" style={{ filter: 'brightness(20%)' }} />
            <div className="w-full absolute px-4 pt-5 rounded-b-lg flex-col bottom-0 items-center text-white
             bg-gradient-to-t from-[rgba(0,0,0,1)] via-[rgba(0,0,0,0.6)] to-transparent">
              <div className="mb-2">
                <h2 className="text-lg">{mission.title}</h2>
                <p className="text-sm">เสาชิงช้า</p>
              </div>
            </div>
            <img
              src={mission.url}
